...
for($pending_request = $inc; $pending_request <= FILE_DOMAINS; $pending_request += $inc ) {
  ...
  for($try = 0; $try < $count_try; $try++) {
    $res = ex(ABSOLUTE_PATH.'/dist/Release/GNU-Linux/dns_checker -b -n'.$pending_request.' '.TEST_FILE.' 2>/dev/null');

    if(preg_match_all("/(\d+);/", $res, $match)) {
      $summ += $match[1][1];
    }
  }
...
?>
  <html>
  <head>
    <title>Gar|k DNS resolver benckmark</title>
